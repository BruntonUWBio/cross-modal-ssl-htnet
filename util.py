...
def set_mod_args(args_in):
    '''Splits up arguments for 2 models'''
    args_in1 = copy.deepcopy(args_in)
    dict_out1 = vars(args_in1) # convert NameSpace object to dict to modify variables
    dict_out1['arch'] = dict_out1['arch1']
    dict_out1['imagenet_path'] = dict_out1['imagenet_path1']
    dict_out1['use_ecog'] = dict_out1['use_ecog1']
    dict_out1['rescale_rate'] = dict_out1['rescale_rate1']
    dict_out1['is_supervised'] = False
    
    args_in2 = copy.deepcopy(args_in)
    dict_out2 = vars(args_in2)
    dict_out2['arch'] = dict_out2['arch2']
    dict_out2['imagenet_path'] = dict_out2['imagenet_path2']
    dict_out2['use_ecog'] = dict_out2['use_ecog2']
    dict_out2['rescale_rate'] = dict_out2['rescale_rate2']
    dict_out2['is_supervised'] = False
    return args_in1, args_in2


def set_n_mod_args(args_in):
    '''Splits up arguments for 2 models'''
    n_modalities = len(args_in.imagenet_path)
    
    args_lst = []
    for i in range(n_modalities):
        args_lst.append(copy.deepcopy(args_in))
        dict_out = vars(args_lst[i]) # convert NameSpace object to dict to modify variables
...
def create_ylabel(measure, traintest):
    if measure == 'acc':
        meas_l = traintest+'Accuracy'
    elif measure == 'homogsc':
        meas_l = traintest+'Homogeneity'
    elif measure == 'completesc':
        meas_l = traintest+'Completeness'
    elif measure == 'vscore':
        meas_l = traintest+'V-measure'
    elif measure == 'adjmi':
        meas_l = traintest+'Adj Mutual Information'
    elif measure == 'adjrandsc':
        meas_l = traintest+'Adj rand score'
    else:
        meas_l = None
    return meas_l

def save_params(fid, d_out):
    d_out['F2'] = d_out['F1'] * d_out['D']
    d_out['cont_data'] = False
    pickle.dump(d_out, open(fid, 'wb'))